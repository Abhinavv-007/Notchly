...
@MainActor
final class NotchlyItemViewModel: ObservableObject {
    @Published private(set) var item: NotchlyItem
    ...
    private let selection = NotchlySelectionModel.shared

    init(item: NotchlyItem) {
        self.item = item
    ...
    let selectedItems = selection.selectedItems(in: NotchlyStateViewModel.shared.items)
    ...
    private func createItemProvider(for item: NotchlyItem) -> NSItemProvider {
        switch item.kind {
        case .file:
            let provider = NSItemProvider()
            if let url = NotchlyStateViewModel.shared.resolveAndUpdateBookmark(for: item) {
    ...
    private func createMultiItemProvider(for items: [NotchlyItem]) -> NSItemProvider {
        let provider = NSItemProvider()
        var urls: [URL] = []
    ...
    func handleClick(event: NSEvent, view: NSView) {
        ...
        if flags.contains(.shift) {
            selection.shiftSelect(to: item, in: NotchlyStateViewModel.shared.items)
    ...
    let selected = NotchlySelectionModel.shared.selectedItems(in: NotchlyStateViewModel.shared.items)
        for it in selected { NotchlyActionService.open(it) }
    }
    ...
    func shareItem(from view: NSView?) {
        Task {
            ...
            if case .text(let text) = item.kind {
                ...
                for item in NotchlySelectionModel.shared.selectedItems(in: NotchlyStateViewModel.shared.items) {
                    switch item.kind {
                    case .file:
    ...
    func presentContextMenu(event: NSEvent, in view: NSView) {
        ...
        let selectedItems = NotchlySelectionModel.shared.selectedItems(in: NotchlyStateViewModel.shared.items)
    ...
    private final class MenuActionTarget: NSObject {
        let item: NotchlyItem
        weak var view: NSView?
        unowned let viewModel: NotchlyItemViewModel
        ...
        init(item: NotchlyItem, view: NSView, viewModel: NotchlyItemViewModel) {
            self.item = item
            self.view = view
        ...
        @MainActor @objc func handle(_ sender: NSMenuItem) {
                ...
                Task {
                    let urls = await selected.asyncCompactMap { item -> URL? in
                        if case .file = item.kind {
                            ...
                            return await NotchlyStateViewModel.shared.resolveAndUpdateBookmark(for: item)
                        }
                        return nil
                    }
                    if !urls.isEmpty {
            ...
            case "Copy":
                ...
                for url in NotchlyItemViewModel.copiedURLs {
                    url.stopAccessingSecurityScopedResource()
                }
                NotchlyItemViewModel.copiedURLs.removeAll()
                
                pb.clearContents()
                Task {
                    let fileURLs = await selected.asyncCompactMap { item -> URL? in
                        if case .file = item.kind {
                            return NotchlyStateViewModel.shared.resolveAndUpdateBookmark(for: item)
                        }
                        return nil
                    }
                    if !fileURLs.isEmpty {
                        // Start security-scoped access for all URLs and keep them active
                        NotchlyItemViewModel.copiedURLs = fileURLs.filter { $0.startAccessingSecurityScopedResource() }
                        NSLog("🔐 Started security-scoped access for \(NotchlyItemViewModel.copiedURLs.count) copied files")
            ...
            case "Remove":
                let selected = NotchlySelectionModel.shared.selectedItems(in: NotchlyStateViewModel.shared.items)
                for it in selected { NotchlyActionService.remove(it) }
                    ...
                    do {
                        ...
                        if let zipTempURL = try await fileURLs.accessSecurityScopedResources(accessor: { urls in
                            ...
                            if let bookmark = try? Bookmark(url: zipTempURL) {
                                let newItem = NotchlyItem(kind: .file(bookmark: bookmark.data), isTemporary: true)
                                NotchlyStateViewModel.shared.add([newItem])
                            } else {
        ...
        @MainActor
        private func showRenameDialog(for item: NotchlyItem) {
                            ...
                            Task {
                                do {
                                    ...
                                    if let newBookmark = try? Bookmark(url: newURL) {
                                        NotchlyStateViewModel.shared.updateBookmark(for: item, bookmark: newBookmark.data)
                                    }
                                } catch {
