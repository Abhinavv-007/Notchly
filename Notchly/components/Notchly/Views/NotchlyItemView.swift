...
struct NotchlyItemView: View {
    let item: NotchlyItem
    @EnvironmentObject var vm: NotchlyViewModel
    @ObservedObject var selection = NotchlySelectionModel.shared
    @StateObject private var viewModel: NotchlyItemViewModel
    ...
    init(item: NotchlyItem) {
        self.item = item
        _viewModel = StateObject(wrappedValue: NotchlyItemViewModel(item: item))
    }
...
private struct DraggableClickHandler<Content: View>: NSViewRepresentable {
    let item: NotchlyItem
    let viewModel: NotchlyItemViewModel
    ...
    final class DraggableClickView: NSView, NSDraggingSource {
        var item: NotchlyItem!
        weak var viewModel: NotchlyItemViewModel?
        ...
        private var draggedItems: [NotchlyItem] = []
        ...
        private func createPasteboardItem(for item: NotchlyItem) -> NSPasteboardItem? {
            let pasteboardItem = NSPasteboardItem()

            switch item.kind {
            case .file:
                guard let url = NotchlyStateViewModel.shared.resolveAndUpdateBookmark(for: item) else {
        ...
        func draggingSession(_ session: NSDraggingSession, willBeginAt screenPoint: NSPoint) {
            NotchlySelectionModel.shared.beginDrag()
        }
        
        
        func draggingSession(_ session: NSDraggingSession, endedAt screenPoint: NSPoint, operation: NSDragOperation) {
            NotchlySelectionModel.shared.endDrag()
            ...
            if Defaults[.autoRemoveNotchlyItems] && !operation.isEmpty {
                for item in draggedItems {
                    NotchlyStateViewModel.shared.remove(item)
                }
            }
