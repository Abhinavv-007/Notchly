//
//  NotchlyPersistenceService.swift
//  boringNotch
//
...
final class NotchlyPersistenceService {
    static let shared = NotchlyPersistenceService()
    ...
    private init() {
        ...
        // Legacy directory name retained from V2 to preserve user shelf state across the rebrand. Do not rename.
    ...
    func load() -> [NotchlyItem] {
        guard let data = try? Data(contentsOf: fileURL) else { return [] }
        
        // Try to decode as array first (normal case)
        if let items = try? decoder.decode([NotchlyItem].self, from: data) {
            return items
        }
