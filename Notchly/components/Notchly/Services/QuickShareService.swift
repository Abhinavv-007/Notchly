...
private class SharingServiceDelegate: NSObject {}
    ...
    func shareDroppedFiles(_ providers: [NSItemProvider], using shareProvider: QuickShareProvider, from view: NSView?) async {
        ...
        for provider in providers {
            if let webURL = await provider.extractURL() {
                ...
                let resolvedURL = await resolveNotchlyItemBookmark(for: itemFileURL) ?? itemFileURL
    ...
    private func resolveNotchlyItemBookmark(for fileURL: URL) async -> URL? {
        let items = await NotchlyStateViewModel.shared.items

        for itm in items {
            if let resolved = await NotchlyStateViewModel.shared.resolveAndUpdateBookmark(for: itm) {
