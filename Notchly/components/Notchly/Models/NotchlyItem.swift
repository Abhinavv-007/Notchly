...
enum NotchlyItemKind: Codable, Equatable, Sendable {
...
@MainActor
struct NotchlyItem: Identifiable, Codable, Equatable, Sendable {
    let id: UUID
    var kind: NotchlyItemKind
    var isTemporary: Bool
    init(id: UUID = UUID(), kind: NotchlyItemKind, isTemporary: Bool = false) {
        self.id = id
        self.kind = kind
    ...
    var fileURL: URL? {
        ...
        return NotchlyStateViewModel.shared.resolveFileURL(for: self)
    }
    ...
    var icon: NSImage {
        ...
        if let resolvedURL = NotchlyStateViewModel.shared.resolveFileURL(for: self) {
...
private extension NotchlyItemKind {
