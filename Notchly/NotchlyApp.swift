...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Notchly", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: NotchlyViewModel] = [:] // UUID -> NotchlyViewModel
    var window: NSWindow?
    let vm: NotchlyViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? NotchlySkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? NotchlySkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createNotchlyWindow(for screen: NSScreen, with viewModel: NotchlyViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = NotchlySkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createNotchlyWindow(
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = NotchlyViewModel(screenUUID: uuid)
                    let window = createNotchlyWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createNotchlyWindow(for: selectedScreen, with: vm)
            }
